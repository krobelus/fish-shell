...
std::vector<int> parse_util_compute_indents(const wcstring &src) {
    ...
    //                 //    cas
    //                 //
    //                 // fish will see that 'cas' is not valid inside a switch statement because it
    //                 is
    //                 // not "case". It will then unwind back to the top level job list, producing
    //                 a
    //                 // parse tree like:
    //                 //
    //                 //   job_list
    ...
    //                 //         cas
    //                 //
    //                 // And so we will think that the 'cas' job is at the same level as the
    //                 switch.
    //                 // To address this, if we see that the switch statement was not closed, do
    //                 not
    //                 // decrement the indent afterwards.
    //                 inc = 1;
    //                 dec = node.parent()->as_switch_statement()->end.unsourced ? 0 : 1;
    //                 break;
    ...
    //         // If we increased the indentation, apply it to the remainder of the string, even if
    //         the
...
static bool detect_errors_in_backgrounded_job(const ast::job_pipeline_t &job,
    ...
    // if foo & ; end
    // while foo & ; end
    const job_conjunction_t *job_conj = job.parent()->try_as_job_conjunction();
    if (!job_conj) return false;

    if (job_conj->parent()->try_as_if_clause()) {
        errored = append_syntax_error(parse_errors, source_range.start, source_range.length,
                                      BACKGROUND_IN_CONDITIONAL_ERROR_MSG);
    } else if (job_conj->parent()->try_as_while_header()) {
        errored = append_syntax_error(parse_errors, source_range.start, source_range.length,
                                      BACKGROUND_IN_CONDITIONAL_ERROR_MSG);
    } else if (const ast::job_list_t *jlist = job_conj->parent()->try_as_job_list()) {
...
static bool detect_errors_in_decorated_statement(const wcstring &buff_src,
    ...
    const statement_t *st = dst.parent()->as_statement();

    // Walk up to the job.
    const ast::job_pipeline_t *job = nullptr;
    for (auto cursor = dst.parent(); job == nullptr; cursor = cursor->parent()) {
    ...
    if (!unexp_command.empty()) {
        ...
        if (!errored && (command == L"break" || command == L"continue") && !first_arg_is_help) {
            ...
            for (auto ancestor = dst.ptr(); ancestor->has_value(); ancestor = ancestor->parent()) {
                ...
                if (!block) continue;
                if (block->header().typ() == type_t::for_header ||
                    block->header().typ() == type_t::while_header) {
                    // This is a loop header, so we can break or continue.
                    found_loop = true;
                    break;
                } else if (block->header().typ() == type_t::function_header) {
...
parser_test_error_bits_t parse_util_detect_errors(const ast::ast_t &ast, const wcstring &buff_src,
    ...
    //     if (const job_continuation_t *jc = node.try_as_job_continuation()) {
    ...
    //         }
    //     } else if (const ast::decorated_statement_t *stmt = node.try_as_decorated_statement()) {
    //         errored |= detect_errors_in_decorated_statement(buff_src, *stmt, &storage,
    //         out_errors);
    ...
    //         errored |= detect_errors_in_block_redirection_list(block->args_or_redirs,
    //         out_errors);
    ...
    //         errored |= detect_errors_in_block_redirection_list(switchs->args_or_redirs,
    //         out_errors);
    //     }
    // }
