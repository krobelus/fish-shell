...
impl IoBufferfill {
    ...
    pub fn flush(&self) {
        fd_monitor().with_fd(self.item_id, |fd| self.buffer.flush(fd));
    }
...
impl IoBuffer {
    ...
    pub fn flush(&self, fd: &AutoCloseFd) {
        let mut locked_buff = self.0.lock().unwrap();
        self.flush_impl(fd, &mut locked_buff);
    }

    fn flush_impl(&self, fd: &AutoCloseFd, locked_buff: &mut MutexGuard<'_, SeparatedBuffer>) {
