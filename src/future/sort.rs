// FROM library/alloc/src/slice.rs

use std::cmp::Ordering;

trait SortBy<T> {
    fn sort_by_strict_weak_order<F>(&mut self, compare: F)
    where
        F: FnMut(&T, &T) -> Ordering;
}

impl<T> SortBy<T> for [T] {
    fn sort_by_strict_weak_order<F>(&mut self, mut compare: F)
    where
        F: FnMut(&T, &T) -> Ordering,
    {
        stable_sort(self, |a, b| compare(a, b) == Ordering::Less);
    }
}
...
mod sort {
    // FROM library/core/src/slice/sort.rs
        ...
        F: FnMut(&T, &T) -> bool,
    {
        ...
        loop {
            ...
            if start_l == end_l {
                ...
                start_l = MaybeUninit::as_mut_ptr(&mut offsets_l[0]) as *mut u8;
                end_l = start_l;
                let mut elem = l;
            ...
            if start_r == end_r {
                ...
                start_r = MaybeUninit::as_mut_ptr(&mut offsets_r[0]) as *mut u8;
                end_r = start_r;
                let mut elem = r;
