...
// Need std::less<> to enable overloads, see https://en.cppreference.com/w/cpp/container/map/find
typedef std::map<wcstring, env_var_t, std::less<>> var_table_t;
...
class environment_t {
   ...
   public:
    virtual maybe_t<env_var_t> get(wcstring_view key,
...
class env_stack_t final : public environment_t {
   ...
   public:
    ...
    /// variable.
    int set(wcstring_view key, env_mode_flags_t mode, wcstring_list_t vals,
            std::vector<event_t> *out_events = nullptr);

    /// Sets the variable with the specified name to a single value.
    int set_one(wcstring_view key, env_mode_flags_t mode, wcstring val,
                std::vector<event_t> *out_events = nullptr);

    /// Sets the variable with the specified name to no values.
    int set_empty(wcstring_view key, env_mode_flags_t mode,
                  std::vector<event_t> *out_events = nullptr);
    ...
    int remove(wcstring_view key, int mode, std::vector<event_t> *out_events = nullptr);
