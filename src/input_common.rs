...
pub trait InputEventQueuer {
    ...
    fn parse_csi(&mut self, buffer: &mut Vec<u8>) -> Option<KeyEvent> {
        ...
        let masked_key = |codepoint, shifted_codepoint: Option<char>| {
            ...
            // fish to match "bind +".
            //
            // But with letters that are affected by capslock, capslock and shift cancel each
            // other out ("capslock-shift-ä"), unless there is another modifier to imply that
            // capslock should be ignored.
            // Consume shift in this case, to make it match "bind ä".
            //
            // We assume the shifted codepoint is absent iff we are dealing with a letter that
            // is affected by capslock (see https://github.com/kovidgoyal/kitty/issues/8493).
            // If this assumption breaks, we'd need to check if the codepoint has an uppercase
            // version (which may depend on the keyboard layout, I'm not sure).
            //
            // Alternatively, we could make "bind ä" match the "shift-ä" event, as suggested
            // in that issue.
            if caps_lock && modifiers == Modifiers::SHIFT && shifted_codepoint.is_none() {
