...
static wcstring get_function_body_source(const function_properties_t &props) {
    ...
    if (props.func_node->header().try_source_range() && props.func_node->end().try_source_range()) {
        auto header_src = props.func_node->header().source_range();
...
int function_properties_t::definition_lineno() const {
    ...
    wcstring source = parsed_source->src();
