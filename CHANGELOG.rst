...
If you want to make sure that completions for "foo" are loaded, use ``complete -C"foo " >/dev/null``  instead).
These files are still exposed via :ref:`status subcommands <status-get-file>`,
mainly for fish's internal use, but
you can also use them as a last resort.
