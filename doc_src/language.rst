...
.. envvar:: fish_terminal_color_theme
   ...
   set to ``light`` or ``dark`` when the terminal uses a light or dark theme respectively;
   ...
   This is used in an :ref:`--on-variable event handler <event>` to update :ref:`syntax highlighting <syntax-highlighting>` whenever the terminal's theme changes.
