...
.. list-table::
   ...
   * - .. _term-compat-cursor-position-report:
     ...
     - Request cursor position report.
       ...
       where the first parameter is the offset from the top row
       and the second parameter is the offset from the leftmost column.

       This is required for terminals that either

       - implement the OSC 133 `click_events <#term-compat-osc-133>`_ feature.
       - advertise the `indn <#term-compat-indn>`_ capability via `XTGETTCAP <#term-compat-xtgettcap>`_
     - VT100
   * - ``\e[ \x20 q``
     - Se
   ...
   * - .. _term-compat-osc-133:
     ...
     - Mark prompt start (OSC 133), with kitty's ``click_events`` extension.
       ...
       the `cursor position reporting <#term-compat-cursor-position-report>`_ feature is required.
     - FinalTerm, kitty
   ...
   * - .. _term-compat-xtgettcap:
     ...
     - Request terminfo capability (XTGETTCAP).
       ...
       Currently, fish only queries the `indn <#term-compat-indn>`_ string capability.
