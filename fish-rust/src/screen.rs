...
impl<'a> Screen<'a> {
    ...
    fn desired_append_char(
        ...
        if b == '\n' {
            ...
            if screen_width.is_none() || (self.desired.cursor.x + cw) > screen_width.unwrap() {
            ...
            if screen_width.is_none() || self.desired.cursor.x >= screen_width.unwrap() {
    ...
    fn scoped_buffer(&mut self) -> impl ScopeGuarding<Target = &mut Self> {
    ...
    fn update(&mut self, left_prompt: &wstr, right_prompt: &wstr, vars: &dyn Environment) {
        ...
        if left_prompt != zelf.actual_left_prompt {
            ...
            for line_break in left_prompt_layout.line_breaks {
                ...
                zelf.outp.tputs_if_some(&term.clr_eol);
                start = line_break;
            }
        ...
        for i in 0..zelf.desired.line_count() {
            ...
            loop {
                    ...
                    && (done || Some(j + 1) == screen_width)
                {
                    ...
                    zelf.write_mbs_if_some(&term.clr_eos);
